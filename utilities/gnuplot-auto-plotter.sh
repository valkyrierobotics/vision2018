...
usage()
{
    echo "Usage: $0 [-f filename] [-n number] [-c channels] [-m max]";
    echo ""
...
parse_opts()
{
    ...
    while getopts "f:n:c:m:h" opt; do
        case $opt in
            f)
                filename=$OPTARG
                ;;
...
if [[ $filename == "../logs/processed_data.log" ]]; then
    gnuplot -e "FILENAME='$filename'; DATA_NUM='$number'; PORT_NUM='$channels'; MAX_SENSOR_VALUE=$max" gnuplot-vision-data.plt
else
    gnuplot -e "FILENAME='$filename'; DATA_NUM='$number'; PORT_NUM='$channels'; MAX_SENSOR_VALUE=$max" gnuplot-realtime-plot.plt
fi
